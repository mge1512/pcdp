...
go mod tidy
go build ./...
go vet ./...                 # part of the gate; must pass
...
## Format-string discipline for templated identifiers (MAC, UUID, port)

`go vet ./...` is part of the compile gate (cli-tool template, Compile
gate Step 2): a printf verb/type mismatch is a gate failure, not a
style nit.

The defect class this guards against: passing a numeric value to a
`%s` verb renders Go's error token into the output instead of the
intended text, e.g. `%!s(int=201)`. In one real run this produced a
wrong-length UUID and a malformed MAC address inside qemu arguments;
qemu rejected the VM only after the tool had already reported success
in the background. The translator's own checks and the gate both
missed it because the code compiled cleanly - only `go vet` flags it.

Rules:

- When filling string templates from numeric values, render the value
  to the intended string form first, then substitute strings into
  strings. Example: render a port number as four uppercase hexadecimal
  digits (`fmt.Sprintf("%04X", port)`), split into two two-character
  halves, and substitute those halves - never hand the raw int to a
  `%s` verb.
- After assembling a templated identifier with a defined grammar (MAC
  address, UUID), validate the assembled string against that grammar
  before passing it to an external command. A malformed identifier
  must fail inside the tool with a diagnostic, not inside qemu (or any
  other consumer) after the tool reported success.
