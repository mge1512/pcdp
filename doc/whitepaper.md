...
**Spec Kitty, Robert Douglass (Industry, 2026)**
Spec Kitty is an open-source (MIT) command-line tool that wraps a governed workflow around AI coding agents: charter, specify, plan, tasks, implement, review, accept, merge, retrospective. It originated as a fork of GitHub Spec Kit and has diverged substantially. Work is cut into packages with lifecycle lanes, each executed in an isolated git worktree, with command surfaces generated for roughly fifteen agent front ends. Two constructs account for most of the design: the *charter*, a per-project governance document recording policy an agent cannot infer from code, and *cross-review with a fresh context*, in which a second model reviews an implementation on the argument that the model which just wrote the code has spent an hour convincing itself the code is correct \[Douglass2026\].

Key differences from Post-Coding Development: Spec Kitty is the articulated rejection of the premise PCD is built on, and its author states the disagreement in print. On his reading, Spec Kit places the specification at the centre as the source of truth for what the software is; he rejects that, because the code is what compiles, what runs, and what customers experience. In his model the specification is a change request, a decision ledger and a guardrail - the source of truth for where the software is going, not for what it is. Every other difference follows from that one choice. The merged code remains the audited object, so the evaluated artefact keeps moving with each change; no attestation of derivation is documented, so provenance is the git history and the decision ledger rather than a hash chain; the target language is a property of the existing repository, so language neutrality has no meaning in that system; and distribution is through a language package index rather than signed packages from a controlled build service. The cross-review instinct is shared, but placed differently: Spec Kitty's reviewer reads a diff and may patch it, while PCD's second model authors an independent test suite from EXAMPLES before the translator writes anything and may only propose changes to the specification, hints, template or prompt. On certification, the honest statement is that both routes are open and distribute the evidence differently: hand-maintained code with an AI agent in the development environment is a familiar and provable arrangement whose cost is that every change re-enters code-level review scope, whereas PCD trades code-level review effort for provenance and pipeline evidence and depends on a certifier accepting that trade. The two target different starting conditions and are complementary: Spec Kitty answers the brownfield case where component behaviour cannot be written down in full, which is precisely where PCD has no answer. A full treatment is in `doc/comparisons/PCD-vs-Spec-Kitty.md`.
...
| Spec Kitty | Charter + change spec (code stays authoritative) | Yes (agent orchestration) | No | No | No |
...
- \[Douglass2026\] Robert Douglass, *"Why I Built Spec Kitty"*, Medium, 27 April 2026. https://medium.com/@robert_98107/why-i-built-spec-kitty-322053bdb992 Project: https://github.com/Priivacy-ai/spec-kitty
...
|---------|------|---------|
| 0.4.6 | 2026-09-01 | A.12: Spec Kitty (Robert Douglass, industry, 2026) added to *Closest Existing Approaches* (after AIUP) and to *Comparative Summary*. First entry in A.12 that rejects rather than converges on the spec-as-source-of-truth premise; the entry records the disagreement as stated by its author and derives the remaining differences from it. Certification wording corrected across the entry: both routes to certification are open and distribute the evidence differently; the earlier framing implied hand-maintained AI-generated code closes the path. New reference \[Douglass2026\]. Full treatment in the new `doc/comparisons/PCD-vs-Spec-Kitty.md`. Mirror update applied to `doc/technical-reference.md` section 18 Related Work. |
